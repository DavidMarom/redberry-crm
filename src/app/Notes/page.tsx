...
const NotesPage = () => {
    ...
    const submitHandler = () => { 
        setDataState([...dataState, { text: input, owner: uid }]);
                ...
                </div>
                {
                    dataState && dataState.map((note: any) => (
                        <div className='grid-item' key={note._id}>
                            <button className="row-r" onClick={() => {
                                setDisabled(true);
                                setDataState(dataState.filter((item: any) => item._id != note._id));
                                deleteMutation.mutate(note._id)
                            }}><Image src="icons/trash.svg" alt="Delete" width={18} height={18} /></button>
